...
public:
	...
	/** Потолок памяти под геометрию бейка, в мегабайтах. BakeCellsToMesh()
	 *  сначала точно считает число наружных граней (без единой аллокации под
	 *  геометрию, см. CellMeshBuilder::CountExposedFaces()), оценивает объём и
	 *  ОТКАЗЫВАЕТСЯ, если он выходит за этот бюджет.
	 *
