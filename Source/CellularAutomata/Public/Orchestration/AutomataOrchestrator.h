...
public:
	 ...
	 *   1. останавливает симуляцию - кадр должен быть неподвижен, иначе тайлы
	 *      снимутся с разных поколений и склеятся в мешанину;
	 *   2. снимает ВСЕ отсечения, включая те, которых нет в профиле (срез J и
	 *      фильтр по возрасту) - "все клетки в кадре" это и есть смысл режима;
	 *   3. применяет профиль Photo (последний в RenderPresets::GetAll());
	 *   4. перерисовывает сетку целиком, одним выстрелом;
	 *   5. выдаёт HighResShot.
	 *
	...
	/** Сколько кадров движок выждет перед снимком (r.HighResScreenshotDelay).
	 *
	 *  Не косметика: за эту паузу успевают достроиться вещи, которые в обычной
	 *  игре подтягиваются за несколько кадров и на глаз незаметны - потоковые
	 *  текстуры, дерево кластеров HISM, накопление Lumen. На неподвижной сцене
	 *  пауза бесплатна, а на снимке разница видна. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Automata|Photo", meta = (ClampMin = "0", UIMax = "120"))
	int32 PhotoShotDelayFrames = 30;
