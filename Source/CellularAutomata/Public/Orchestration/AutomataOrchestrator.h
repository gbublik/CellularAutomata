...
public:
	 ...
	 *  затирать друг друга.
	 *
	 *  Направляет в HUD (UMainHudWidget::OnStatusMessage()), а в движковый
	 *  AddOnScreenDebugMessage() падает только если HUD принять не может -
	 *  виджета ещё нет (вне PIE, до InitializeHUD()) либо событие не разведено
	 *  в Blueprint-наследнике. Движковый канал рисует строки от зашитых
	 *  константой 45 пикселей сверху (UnrealEngine.cpp, MessageStartY - ни
	 *  настройки, ни cvar наружу не выведено), т.е. поверх верхней панели HUD;
	 *  ради этого маршрут и заведён. То, что это ЕДИНСТВЕННЫЙ на весь проект
	 *  вызов AddOnScreenDebugMessage(), и сделало правку правкой одного места -
	 *  см. абзац выше про то, почему метод публичный. */
	 ...
	 *
	 *  Включение ещё и ЗАПУСКАЕТ прогон, если тот стоит (Start()): на паузе
	 *  перебирать нечего, вымирает только то, что считается. Выключение прогон
	 *  не останавливает - "хватит перебирать" не означает "хватит считать". */
	UFUNCTION(BlueprintCallable, Category = "Automata")
	 ...
	 *
	 *  Этими же параметрами строится состояние при СТАРТЕ ИГРЫ: BeginPlay()
	 *  зовёт GenerateState(), а не безусловный случайный шар - иначе настроенная
	 *  здесь решётка при запуске молча подменялась бы кубическим шаром. Прежнее
	 *  поведение получается выбором EStateGeneratorType::RandomBall, отдельного
	 *  переключателя для этого не нужно.
	 *
	 *  Отдельно от блока Automata|Random: тот описывает случайный шар, который
	 *  по-прежнему строят N (NewSeed) и запасной путь R, и который обязан
	 *  остаться собой независимо от того, что здесь накручено. То есть после
	 *  этой правки старт и N могут дать РАЗНЫЕ фигуры - если это мешает,
	 *  ставьте здесь RandomBall либо переводите NewSeed() на GenerateState()
	 *  тоже. */
	...
	/** Подвинуть призрак на клетку по экранному направлению (стрелки в режиме
	 *  рисования): (1,0) - вправо, (0,1) - вверх. Возвращает false, если двигать
	 *  нечего (призрака нет).
	 *
	 *  ЗАЧЕМ. На сложной поверхности попасть мышью в нужную клетку тяжело:
	 *  соседние грани отличаются на несколько пикселей, а прицел ещё и пляшет
	 *  вместе с рукой. Стрелки дают ровно то, чего не даёт мышь, - шаг
	 *  дискретный и точный.
	 *
	 *  ЭКРАННОЕ, А НЕ МИРОВОЕ направление, и это не удобство, а условие
	 *  работоспособности: клавиша "вправо" обязана двигать вправо НА ЭКРАНЕ,
	 *  куда бы ни смотрела камера. Ось выбирается перебором шагов решётки по
	 *  наибольшей проекции на экранное направление (см. реализацию), поэтому
	 *  приём одинаково верен и на кубе, и на скошенных решётках, где "ось Y" на
	 *  экране вообще не горизонтальна.
	 *
	 *  Шаг идёт В ПЛОСКОСТИ ГРАНИ, к которой призрак прилип: ось нормали из
	 *  перебора выбрасывается, иначе стрелка отрывала бы призрак от
	 *  поверхности. Когда нормали нет (ставим в пустоту), доступны все три оси.
	 *
	 *  Пока сдвиг активен, призрак НЕ следует за курсором - см.
	 *  ClearCellPreviewNudge(). */
	bool NudgeCellPreview(int32 ScreenRight, int32 ScreenUp);

	/** Вернуть призрак под курсор. Зовётся при движении мыши (и при выходе из
	 *  режима): мышь всегда главнее, стрелки - её уточнение, а не замена. */
	void ClearCellPreviewNudge();

	/** Активен ли сдвиг стрелками - HUD'у и на всякий случай наружу. */
	bool IsCellPreviewNudged() const { return bPreviewNudgeActive; }
	...
	/** Повернуть буфер на 90 градусов вокруг оси (0 = X, 1 = Y, 2 = Z) - см.
	 *  CellClipboard::Rotate90(). Крутится САМ буфер, а не его отображение:
	 *  вставится ровно то, что показывает призрак, и поворот переживает
	 *  сколько угодно вставок подряд.
	 *
	 *  Инстансы призрака после поворота перезаливаются - это единственный
	 *  случай, когда они пересобираются не в момент копирования, и он того
	 *  стоит: без перезаливки призрак показывал бы старую ориентацию. */
	...
	/** Проверяет, что вокруг генератора паттернов 2D-жизни стоят настройки, при
	 *  которых перенос вообще работает (толщина 2, окрестность Moore, States 2,
	 *  правило 5,7/6, выключенный фильтр чётности), и пишет расхождения в лог и
	 *  на экран.
	 *
	 *  Нужна потому, что отказ здесь МОЛЧАЛИВЫЙ: паттерн строится и выглядит
	 *  правильно, но не оживает - ни ошибки, ни подсказки, что искать. Сам
	 *  генератор остаётся чистой геометрией и правило не читает (см.
	 *  StateGenerators), а оркестратор видит обе стороны и потому обязан
	 *  сказать. */
	void WarnIfLifePatternMismatch();
	 ...
	 *  ЖИВОЙ структуры (Ctrl+Y, см. AnalyzeLiveStructure()). Не украшение и не
	 *  экономия памяти: стоимость растёт как выборка * соседей^2 (для каждой
	 *  клетки выборки обходятся её соседи, и для каждого пустого соседа - его
	 *  собственные), так что при 24 это ~117 тыс. клеток и порядка секунды, а
	 *  при 48 - уже почти миллион и больше десяти секунд. Поднимать имеет
	 *  смысл, когда структура крупная и ответ по её ядру не похож на правду;
	 *  сколько клеток реально попало в выборку, всегда пишется в лог. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Automata|Debug",
			  meta = (ClampMin = "1", UIMin = "4", UIMax = "64"))
	int32 LiveAnalysisSampleExtent = 24;

	/** Гистограмма живых соседей по ТЕКУЩЕЙ структуре: сколько живых клеток
	 *  видит ровно N живых соседей и сколько примыкающих пустых видит N.
	 *  Печатается в лог, а следом - строка ИТОГО, посчитанная по действующим
	 *  Survival/Birth: сколько клеток обречено, сколько родится, нетто и во
	 *  сколько раз вырастет население. Ради неё функция и нужна: гистограмма
	 *  отвечает "какое распределение", сводка - "куда по нему бьют пороги".
	 *  На экран уходит только сводка, гистограмма для этого слишком длинная.
	 *
	 *  Пара к тому, что печатает GenerateState() для только что построенного
	 *  набора (FStateGeneratorParams::bAnalyzeNeighborCounts): та же мера,
	 *  снятая не до эволюции, а посреди неё. Отвечает ровно на вопрос "куда
	 *  относительно реального распределения попадают пороги Survival/Birth" -
	 *  скажем, при Survival 0-6 из 26 соседей видно, какая доля клеток уже
	 *  перевалила за 7 и обречена вымереть на следующем шаге.
	 *
	 *  Считается по центральному подкубу (LiveAnalysisSampleExtent), потому
	 *  что стоимость квадратична по числу соседей - для эволюционировавшей
	 *  структуры это ЯДРО, а не фронт роста, и на неоднородной структуре
	 *  ответы для ядра и для фронта будут разными. Размер выборки печатается
	 *  рядом с гистограммой, чтобы это было видно, а не подразумевалось. */
	UFUNCTION(BlueprintCallable, CallInEditor, Category = "Automata|Debug")
	void AnalyzeLiveStructure();
	...
	/** Быстро сохранить НАХОДКУ (хоткей Alt+S): новый файл со сгенерированным
	 *  именем, без диалога и без вопросов.
	 *
	 *  Третий член семейства S, и заведён он потому, что первые два решают
	 *  другую задачу. Ctrl+S тихо ПЕРЕЗАПИСЫВАЕТ LastSaveFilePath - правильная
	 *  семантика для документа, который правят, и ровно неправильная для
	 *  коллекции: сохранил находку, через полчаса нашёл вторую, нажал Ctrl+S -
	 *  и первой больше нет. Ctrl+Shift+S ничего не теряет, но открывает
	 *  МОДАЛЬНЫЙ диалог и требует придумать имя - то есть прерывает ровно в тот
	 *  момент, когда на экране происходит интересное.
	 *
	 *  Отсюда все три отличия: всегда новый файл, никогда не диалог, и
	 *  LastSaveFilePath НЕ трогается (через WriteStateToFile(..., false)) -
	 *  быстрое сохранение не должно менять то, куда потом целится Ctrl+S.
	 *
	 *  Имя собирается из действующего правила, номера поколения и времени:
	 *  "Find_5,6-4-2-M_gen137_2026.08.08-19.42.11.casave". Косые из правила
	 *  заменены на дефисы (в имени файла они незаконны) - та же запись, что
	 *  пользователь уже применял в именах своих находок. Номер поколения не
	 *  украшение: в файл идёт ИЗНАЧАЛЬНЫЙ паттерн, а не то, что на экране, так
	 *  что без "досмотреть до 137-го" находка воспроизводима лишь наполовину.
	 *
	 *  Форму клетки восстанавливать не нужно и она не сохраняется: она
	 *  ВЫВОДИТСЯ из четырёх полей решётки, а те в шапке есть - при загрузке
	 *  SyncCellShapeFromLatticeFields() вернёт и решётку, и меш. */
	UFUNCTION(BlueprintCallable, CallInEditor, Category = "Automata|SaveLoad")
	void QuickSaveFind();
	 ...
	 *
	 *  Камера НЕ трогается (как и при R - автокадрирование убрано сознательно,
	 *  см. AGamePlayerController::FrameAllCells()); в кадр загруженное попадает
	 *  потому, что в файле узор лежит вокруг НУЛЯ - его туда переносит запись
	 *  (см. WriteStateToFile()). Файлы, записанные до появления переноса,
	 *  по-прежнему грузятся там, где были сохранены, - лечится одним Home и
	 *  повторным Ctrl+S, который перезапишет их уже центрированными. */
	UFUNCTION(BlueprintCallable, CallInEditor, Category = "Automata|SaveLoad")
	...
	/** Хоткей Ctrl+Z: показать ПРЕДЫДУЩЕЕ поколение - не доставая его из
	 *  истории, а пересчитав заново от изначального узора до GenerationCount-1.
	 *
	 *  Историю состояний проект не хранит и хранить не будет: одно поколение на
	 *  этих масштабах - сотни мегабайт, а откат нужен изредка и вручную. Зато
	 *  есть всё, чтобы его воспроизвести - InitialStateCells (точка возврата R)
	 *  и детерминированное правило, - поэтому "отмена" здесь это пересев плюс
	 *  фоновый прогон, ровно тот же цикл StepBatch(), что в Next(), только
	 *  длиной GenerationCount-1 и без единого рендера промежуточных поколений.
	 *  Рендер и есть основная цена кадра (ради этого существует StepsPerRender),
	 *  так что слепой пересчёт сотни поколений заметно дешевле, чем те же сто
	 *  поколений, прожитые на экране.
	 *
	 *  Обратного шага у автомата НЕ существует: правило не биекция, у состояния
	 *  бывает много предков или ни одного (сады Эдема). Поэтому "назад" может
	 *  означать только "посчитать заново меньше", и цена нажатия линейна по
	 *  номеру поколения, а не постоянна.
	 *
	 *  ЧЕГО ЖДАТЬ, ЕСЛИ ПРАВИЛО ПРАВИЛИ ПО ХОДУ ПРОГОНА: пересчёт идёт ТЕКУЩИМ
	 *  правилом (BuildRule() читает Details panel заново, как и везде в проекте),
	 *  а прошлое нигде не записано. Смена правила/окрестности/States посреди
	 *  прогона делает откат не "предыдущим кадром", а "тем, что получилось бы,
	 *  если бы новое правило работало с самого начала". Это осознанно: хранить
	 *  историю правок правила значило бы завести ту самую историю, которой здесь
	 *  сознательно нет.
	 *
	 *  Непрерывный прогон останавливается (Stop()) - иначе следующий же
	 *  StepAsync() затрёт результат отката, и нажатие выглядело бы
	 *  несработавшим. Удержание не повторяется (ловится IE_Pressed, не
	 *  IE_Repeat): каждое нажатие стоит целого прогона, и авторепит на зажатой
	 *  клавише сложился бы в квадрат по номеру поколения. */
	UFUNCTION(BlueprintCallable, CallInEditor, Category = "Automata|Selection")
	void StepBackward();

	/** Хоткей Ctrl+Z целиком: отменить ПОСЛЕДНЕЕ действие, чем бы оно ни было.
	 *
	 *  Одна клавиша на две разные отмены, потому что для пользователя это одно
	 *  действие "верни как было", а не выбор механизма. Разводится по тому,
	 *  что случилось последним:
	 *
	 *  - последняя запись журнала сделана на ТЕКУЩЕМ поколении - значит после
	 *    неё симуляция не шагала, и отменяется именно она: обратная дельта
	 *    прямо по сетке, мгновенно, без всякого пересчёта;
	 *  - иначе последним был шаг симуляции - и отменяется он, пересчётом
	 *    (см. StepBackward()).
	 *
	 *  Отсюда естественно получается ожидаемый порядок: правка, шаг, шаг -
	 *  три Ctrl+Z снимут шаг, шаг и правку, в обратном порядке. Отменённая
	 *  правка уходит в EditRedoStack (Ctrl+Shift+Z), отменённое поколение -
	 *  никуда: "повтор шага" это F, и отдельный стек для него был бы вторым
	 *  способом сделать то же самое. */
	UFUNCTION(BlueprintCallable, CallInEditor, Category = "Automata|Selection")
	void UndoLastAction();

	/** Хоткей Ctrl+Shift+Z: вернуть последнюю отменённую правку.
	 *
	 *  Только правки - см. UndoLastAction() о том, почему повтор шага это F.
	 *  Стек повтора гасится любым новым действием, и это не косметика: правка
	 *  помнит поколение, на котором сделана, а после нового шага такого
	 *  поколения в траектории уже нет. */
	UFUNCTION(BlueprintCallable, CallInEditor, Category = "Automata|Selection")
	void RedoLastEdit();

	/** Есть ли что отменять/повторять - для HUD (кнопки и подсказки) и для
	 *  строки статуса. */
	UFUNCTION(BlueprintPure, Category = "Automata|Selection")
	bool CanUndo() const { return EditJournal.Num() > 0 || GenerationCount > 0; }

	UFUNCTION(BlueprintPure, Category = "Automata|Selection")
	bool CanRedo() const { return EditRedoStack.Num() > 0; }
	...
	/** Во сколько раз подсветка выделения крупнее самой клетки. Ровно 1.0
	 *  нельзя: поверхности тогда совпадают и мерцают (z-fighting) - поэтому по
	 *  умолчанию 1.1: подсветка обволакивает клетку и видна с любого угла.
	 *
	 *  Множитель ОТНОСИТЕЛЬНЫЙ: применяется поверх CellMeshScaleMultiplier, а
	 *  не вместо него (см. RenderSelectionOverlay()). Пока тот был единицей,
	 *  разницы не было, и подсветка задавалась абсолютно; на ячейках решёток с
	 *  масштабом 2 (ромбододекаэдр, усечённый октаэдр) это давало подсветку
	 *  вдвое МЕНЬШЕ клетки, целиком спрятанную внутри неё - выделение работало,
	...
	/** ТУМБЛЕР ФОРМЫ КЛЕТКИ. Переключение здесь применяет форму целиком: меш из
	 *  соответствующего слота (см. блок Automata|Cells|Meshes ниже), фильтр
	 *  чётности, окрестность, растяжение решётки по Z и множитель меша - и
	 *  ПЕРЕСОБИРАЕТ сетку, потому что решётка хранится внутри неё.
	 *  Эквивалентно ApplyCellShapePreset() с индексом этой формы; ровно тот же
	 *  путь используют консольная команда CA.CellShape и HUD.
	 *
	 *  BlueprintReadOnly НАМЕРЕННО: запись в поле мимо SetCellShape() поменяла
	 *  бы подпись на тумблере, не тронув ни одного поля, которые форму
	 *  собственно и составляют, - то есть создала бы ровно тот пятый источник
	 *  правды, которого таблица форм избегает (см. FCellShapePreset). Из
	 *  Details panel запись перехватывает PostEditChangeProperty(), из
	 *  Blueprint - зовите SetCellShape().
	 *
	 *  Значение здесь - "какая форма применена последней". Четыре поля,
	 *  составляющие геометрию, остаются обычными и редактируемыми: правка любого
	 *  из них руками законна и просто делает тумблер неактуальным. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Automata|Cells")
	ECellShape CellShape = ECellShape::Cube;

	/** Меш, которым отрисовывается одна клетка (инстансированный) - то, что
	 *  реально читают рендерер, превью-кисть рисования и призрак буфера обмена.
	 *  ЕДИНСТВЕННЫЙ источник для них и остаётся им: слоты ниже не читаются на
	 *  каждый кадр, а лишь подставляются сюда при переключении формы. Тот же
	 *  идиом, что у LatticeZScale - пресет выставил и ушёл, поле осталось
	 *  редактируемым. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Automata|Cells")
	UStaticMesh* CellMesh = nullptr;

	/** Меш для формы "куб" - обычный единичный куб, габарит 1x1x1. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Automata|Cells|Meshes")
	UStaticMesh* CubeMesh = nullptr;

	/** Меш для гексагональной призмы, габарит 1 x 1.1547 x 1. Слот существует
	 *  ради полноты пятёрки Фёдорова, но применить эту форму сейчас нельзя:
	 *  ей нужна скошенная решётка, которой FLatticeTransform не умеет (см.
	 *  CellShapePresets::RequiresShearedLattice()). Меш здесь ничего не
	 *  чинит - сначала сдвиг в решётке, потом ассет. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Automata|Cells|Meshes")
	UStaticMesh* HexagonalPrismMesh = nullptr;

	/** Меш ромбододекаэдра: 14 вершин / 24 ребра / 12 граней, габарит 2x2x2. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Automata|Cells|Meshes")
	UStaticMesh* RhombicDodecahedronMesh = nullptr;

	/** Меш удлинённого додекаэдра: 18 вершин / 28 рёбер / 12 граней, объём
	 *  ровно 8 при габарите 2x2x3, апексы смотрят по Z. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Automata|Cells|Meshes")
	UStaticMesh* ElongatedDodecahedronMesh = nullptr;

	/** Меш усечённого октаэдра: 24 вершины / 36 рёбер / 14 граней, объём ровно
	 *  4 при габарите 2x2x2. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Automata|Cells|Meshes")
	UStaticMesh* TruncatedOctahedronMesh = nullptr;

	/** Меш, назначенный слоту этой формы (nullptr, если слот пуст). Слоты -
	 *  пять отдельных именованных полей, а не массив или TMap, ровно по одной
	 *  причине: в Details panel каждый из них подписан своей фигурой, и
	 *  положить меш не в тот слот заметно сразу, а не по щелям в картинке.
	 *  Цена - вот этот switch, единственное место, где имя формы превращается
	 *  в поле. */
	UFUNCTION(BlueprintPure, Category = "Automata|Cells")
	UStaticMesh* GetCellMeshForShape(ECellShape Shape) const;
	 ...
	 *  он выставляет заодно фильтр чётности, окрестность и множитель меша,
	 ...
	 *
	 *  Своё свойство, а не настройка ассета, потому что рендерер габарит меша
	 *  НОРМАЛИЗУЕТ: FInstancedMeshCellGridRenderer вычисляет
	 *  InstanceScale = FVector(CellSize) / MeshBoundsSize, то есть собственный
	 *  размер меша сокращается и мировой габарит всегда равен
	 *  CellSize * ScaleMultiplier. Как ни авторь ассет, без этого множителя
	 *  клетка будет ровно в CellSize.
	 *
	 *  Нужно это ГЦК-режиму (см. FStateGeneratorParams::ParityFilter): на
	 *  ГЦК-подрешётке соседи стоят на расстоянии sqrt(2)*CellSize, а ячейка
	 *  Вороного - ромбододекаэдр с габаритом РОВНО 2*CellSize (проверка
	 *  объёмом: куб со стороной S плюс шесть пирамид с основанием S^2 и высотой
	 *  S/2 дают 2*S^3, что совпадает с плотностью подрешётки - одна точка на две
	 *  единичные ячейки). То есть меш ромбододекаэдра плюс множитель 2.0 дают
	 *  замощение без щелей; с кубом и множителем 1.0 клетки будут касаться
	 *  только рёбрами.
	 *
	 *  На подсветку выделения влияет тоже: SelectionScaleMultiplier применяется
	 *  ПОВЕРХ этого множителя, а не вместо него - иначе подсветка ячейки с
	 *  масштабом 2 оказалась бы вдвое меньше самой ячейки. Отдельным свойством
	 *  она остаётся потому, что решает свою задачу: её 1.1 борется с
	 *  z-fighting'ом относительно клетки, какого бы та ни была размера. Прежний
	 *  текст этого абзаца утверждал обратное и был неверен - см.
	 *  SelectionScaleMultiplier,
	 *  и он умышленно остаётся отдельным: его 1.1 борется с z-fighting'ом
	 *  относительно клетки, какого бы та ни была размера. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Automata|Cells",
	...
	/** Снимок "для фотки": максимальное разрешение, ничего не отсечено (F10).
	 *
	 *  Отличается от F6/F7 принципиально. Те растеризуют СЕТКУ напрямую,
	 *  минуя GPU: плоская картинка без света, теней и бликов - идеальная маска,
	 *  но не парадный кадр. Здесь наоборот - обычный рендер движка со всем
	 *  освещением, просто в разрешении, которое в окно не помещается.
	 *
	 *  Последовательность (порядок важен, каждый шаг готовит следующий):
	 *   1. останавливает симуляцию - кадр должен быть неподвижен;
	 *   2. применяет профиль съёмки (Photo или Photo Lean);
	 *   3. перерисовывает сетку целиком, одним выстрелом;
	 *   4. выдаёт HighResShot.
	 *
	 *  Снять можно ровно то, что можно показать: кадр идёт через обычный рендер
	 *  движка, то есть через тот же RenderGridImmediate() и те же инстансы.
	 *  Здесь был параметр "промотать N поколений перед снимком" - он снят как
	 *  бессмысленный: состояние, которое не влезает на экран, не влезет и в
	 *  кадр, а промотка к нему только тратила бы время.
	 *
	 *  Настройки РАЗГЛЯДЫВАНИЯ при этом не трогаются вовсе: куб отсечения (C),
	 *  срез вдоль взгляда (J), фильтр по возрасту (цифры) и спрятанный фон (U)
	 *  остаются как есть. Снимают ровно то, что видят. Профиль владеет только
	 *  качеством и двумя ускорителями, которые снимок портят (отсечение по
	 *  расстоянию и Ghost Shape).
	 *
	 *  ВАЖНО про стоимость: HighResShot НЕ тайлит. Он заводит один рендер-
	 *  таргет запрошенного размера (UnrealClient.cpp: DummyViewport->SizeX =
	 *  GScreenshotResolutionX), то есть видеопамять растёт как ПЛОЩАДЬ кадра, а
	 *  не остаётся на уровне окна. 4K это ~8 Мпикс, 8K - уже ~33, и GBuffer под
	 *  них измеряется гигабайтами. Первая версия этого метода вдобавок ставила
	 *  ScreenPercentage 200 (из ложной посылки о тайлах и швах между ними) и
	 *  роняла редактор на сетке в 82 тысячи клеток - размер кадра тут решает
	 *  всё, размер сетки почти ничего. Отсюда предполётная проверка в
	 *  ValidatePhotoShotResolution().
	 *
	 *  Профиль после съёмки НЕ возвращается: кадр обычно приходится
	 *  выстраивать, а видеть при этом надо итоговое качество. Возврат - любая
	 *  из F1-F4. */
	UFUNCTION(BlueprintCallable, CallInEditor, Category = "Automata|Photo")
	void TakePhotoShot();

	/** Разрешение снимка. По умолчанию 4K, а не 8K, и это следствие вылета: кадр
	 *  рендерится целиком, одним таргетом, поэтому видеопамять растёт как
	 *  площадь. 4K (~8 Мпикс) проходит на любой карте, к 8K (~33 Мпикс) стоит
	 *  подниматься, убедившись, что 4K снимается без вылета. Верхний предел
	 *  жёсткий - сторона 2D-текстуры у RHI, обычно 16384; он проверяется до
	 *  съёмки, см. ValidatePhotoShotResolution(). */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Automata|Photo", meta = (ClampMin = "64"))
	FIntPoint PhotoShotResolution = FIntPoint(3840, 2160);

	/** Сколько кадров отрисовать перед тем, как сохранить (r.HighResScreenshotDelay).
	 *
	 *  ВНИМАНИЕ, название обманчиво - это НЕ пауза. Движок крутит цикл
	 *  (UnrealClient.cpp), в котором рисует кадр в ПОЛНОМ разрешении снимка
	 *  ровно столько раз, сохраняя только последний. То есть значение это
	 *  прямой множитель времени съёмки: 8 стоит вдвое дороже, чем 4.
	 *  Цикл синхронный (FlushRenderingCommands() на каждой итерации), так что
	 *  окно всё это время заморожено - промежуточных кадров никто не увидит.
	 *
	 *  Прогоны нужны затем, что часть подсистем достраивается не за один кадр:
	 *  потоковые текстуры, страницы VSM, накопление Lumen и TSR. Отсюда и
	 *  движковое значение по умолчанию - 4, оно же взято здесь. Первая версия
	 *  ставила 30 - тридцать полноразмерных рендеров на один снимок, впустую.
	 *
	 *  Ниже 4 движок принудительно выключает TemporalAA (ShowFlags.cpp: "requires
	 *  multiple frames"). Профилю Photo Lean это безразлично, он и так на FXAA,
	 *  а вот обычному Photo с TSR ниже четырёх опускаться не стоит. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Automata|Photo", meta = (ClampMin = "1", UIMax = "16"))
	int32 PhotoShotDelayFrames = 4;

	/** Снимать экономным профилем (Photo Lean вместо Photo).
	 *
	 *  Отключает подсистемы, чей расход растёт вместе с размером кадра: Lumen,
	 *  Virtual Shadow Maps, историю TSR, пост-обработку. Тени и свет при этом
	 *  остаются - обычные карты теней живут в атласе фиксированного размера и от
	 *  разрешения кадра не зависят.
	 *
	 *  Это то, чем берут разрешения, не влезающие иначе. По умолчанию выключено:
	 *  картинка всё же беднее (нет отражённого света Lumen и bloom), и навязывать
	 *  этот размен там, где памяти хватает, незачем. Расход обоих вариантов
	 *  печатается в лог при съёмке - сравнивать по числу, а не на глаз. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Automata|Photo")
	bool bPhotoLeanMemory = false;

	/** Рисовать только ОБОЛОЧКУ: клетки, закрытые соседями со всех сторон, в
	 *  кадр не идут вовсе.
	 *
	 *  Картинка при этом попиксельно та же - внутренность непрозрачного тела не
	 *  видна ни одним пикселем по определению, - а инстансов становится на
	 *  порядок меньше: поверхность растёт как n^(2/3), и у сплошного куба из 14
	 *  млн клеток оболочка это около 350 тысяч.
	 *
	 *  ЗАЧЕМ ЭТО ВАЖНЕЕ ЭКОНОМИИ ПАМЯТИ. Потолок движка - 16 млн инстансов
	 *  (GPU-Scene, а за ним 24-битный InstanceId в аппаратном ray tracing), и
	 *  это потолок в ШТУКАХ. Ужимать байты на инстанс его не двигает вовсе;
	 *  двигает только уменьшение их числа. Оболочка - единственный способ выйти
	 *  за 16 млн клеток, ничего не потеряв на экране.
	 *
	 *  Выигрыш зависит от структуры: у плотного тела - десятки раз, у губки или
	 *  ветвистой фигуры почти всё и есть поверхность, там выигрыша нет. Но
	 *  упираются в потолок как раз плотные.
	 *
	 *  ОДНО УСЛОВИЕ: материал клетки должен быть НЕПРОЗРАЧНЫМ. Сделаете клетки
	 *  полупрозрачными - выключите этот режим, иначе сквозь стенку будет видно
	 *  пустоту вместо внутренности.
	 *
	 *  Отсечения (срез J, куб C, фильтр возраста) режим учитывает: сосед
	 *  считается закрывающим, только если он сам нарисован (см.
	 *  IsCellEnclosed()), поэтому на плоскости среза внутренность не полая.
	 *
	 *  Побочно меняется смысл числа "отрисовано" в статистике: оно становится
	 *  площадью поверхности, а не числом видимых клеток. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Automata|Rendering")
	bool bRenderSurfaceOnly = true;
	 ...
	 *
	 *  Снимается ШЕСТЬЮ обычными захватами сцены по 90 градусов, а не одним
	 *  кубическим: у кубического захвата ориентация граней задана движком, и
	 *  сшивать его пришлось бы, угадывая эту раскладку, - а тут базис каждой
	 *  грани берётся ИЗ ТОГО ЖЕ поворота, которым грань снята (см.
	 *  BuildPanoramaFaces()), так что перепутать оси негде в принципе.
	 *
	 *  ГОРИЗОНТ ВСЕГДА РОВНЫЙ. Берётся положение камеры и её РЫСКАНИЕ, а тангаж
	 *  и крен отбрасываются: в сферической панораме наклон камеры не
	 *  наклоняет кадр, а заваливает мир целиком - вертикали перестают быть
	 *  вертикальными, и просмотрщик показывает кривую сцену. Рыскание же
	 *  сохраняется, поэтому центр панорамы - то, на что вы смотрели.
	 *
	 *  Экранные эффекты (SSR, SSAO, bloom, виньетка, motion blur, DOF)
	 *  выключены НЕ ради скорости: все они считаются в плоскости кадра, а у
	 *  панорамы кадров шесть - на стыках граней такой эффект обрывается, и шов
	 *  видно тем отчётливее, чем эффект сильнее. Экспозиция по той же причине
	 *  переведена в ручную: автоэкспозиция замерила бы каждую грань отдельно, и
	 *  яркость скакала бы по кругу ступеньками.
	 *
	 *  В отличие от F10 окно не замирает на минуты: шесть граней рисуются
	 *  захватами сцены, а не полноэкранными прогонами. */
	UFUNCTION(BlueprintCallable, CallInEditor, Category = "Automata|Photo")
	void TakePanoramaShot();

	/** Ширина панорамы в пикселях; высота всегда вдвое меньше - у
	 *  равнопромежуточной проекции соотношение сторон ровно 2:1, иначе
	 *  просмотрщик растянет картинку сам.
	 *
	 *  8192x4096 (~34 Мпикс) - обычный размер для панорам, и он ЗДЕСЬ дешевле,
	 *  чем те же 34 Мпикс у F10: кадр не рисуется целиком одним рендер-таргетом,
	 *  память держат шесть квадратных граней стороной Width/4 (плюс множитель
	 *  PanoramaSupersample) и сама выходная картинка в оперативной памяти. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Automata|Photo", meta = (ClampMin = "512", UIMax = "16384"))
	int32 PanoramaWidth = 8192;

	/** Во сколько раз грани снимаются крупнее, чем нужно выходной картинке.
	 *
	 *  Сторона грани подобрана так, что в центре её плотность пикселей РОВНО
	 *  совпадает с плотностью панорамы на экваторе (Width/4 против Width/2pi на
	 *  радиан - те же 1:1). Совпадение точное, и оно же означает, что запаса на
	 *  сглаживание нет никакого: одиночный захват сцены не накапливает историю
	 *  TSR, и ступеньки на рёбрах клеток остаются как есть.
	 *
	 *  Множитель 2 - и есть сглаживание: грань рисуется вчетверо большей
	 *  площадью и усредняется при сшивке. Стоит вчетверо больше времени на
	 *  затенение и вчетверо больше видеопамяти на грань (при 8192 это 4096x4096,
	 *  64 МБ), но грань всё равно на порядок меньше полноразмерного кадра F10.
	 *  1 - без сглаживания, для быстрой проверки ракурса. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Automata|Photo", meta = (ClampMin = "1", ClampMax = "2"))
	int32 PanoramaSupersample = 2;

	/** Поправка экспозиции панорамы, ступеней (EV).
	 *
	 *  Существует потому, что автоэкспозиция здесь запрещена (иначе яркость
	 *  скачет от грани к грани), а ручная не обязана совпасть с тем, к чему
	 *  привык глаз в окне: окно свою экспозицию подбирает само. Если панорама
	 *  вышла темнее или светлее, чем вид, - это тот самый разрыв, и правится он
	 *  здесь. Плюс - светлее, минус - темнее, шаг в единицу это вдвое. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Automata|Photo", meta = (UIMin = "-8", UIMax = "8"))
	float PanoramaExposureBias = 0.0f;
...
public:
	...
	/** Какие клетки вокруг считаются соседними - набор оболочек, от 6 соседей
	 *  (VonNeumann) до 26 (Moore, EdgesCornersFarAxes). Полный список и что
	 ...
	 *  Пока здесь Shells (дефолт), окрестность берётся из Neighborhood выше;
	 ...
	 *  выбранная окрестность просто исчезает без объяснений. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Automata|Rules")
	 ...
	 *  чётности, окрестность, растяжение решётки и множитель меша - все четыре
	...
	/** То же самое, но по устойчивому имени формы, - путь тумблера CellShape,
	 *  Blueprint и FRulePreset::RequiredCellShape. Форма, которой нет в
	 *  таблице, - warning в лог, ничего не меняется. */
	UFUNCTION(BlueprintCallable, Category = "Automata|Cells")
	void SetCellShape(ECellShape NewShape);

	/** Подгоняет тумблер CellShape (и вместе с ним меш и множитель меша) под
	 *  четыре поля, которые геометрию собственно и составляют, - НЕ трогая
	 *  сетку.
	 *
	 *  Нужно после загрузки .casave: файл хранит фильтр чётности, растяжение по
	 *  Z и окрестность-списком, но не форму и не меш (см. FAutomatonSaveHeader -
	 *  ассеты туда сознательно не входят). Без этой синхронизации загруженная
	 *  ОЦК-структура рисовалась бы кубами, а тумблер показывал бы форму от
	 *  прошлого раза - то есть врал бы.
	 *
	 *  Перегенерации здесь быть не может в принципе: её вызов стёр бы только
	 *  что загруженный паттерн. Комбинацию полей, которой не соответствует ни
	 *  одна форма, оставляет как есть - это законное состояние (форму собрали
	 *  руками), а не ошибка. */
	 ...
	 *
	 *  ЕДИНСТВЕННОЕ ИСКЛЮЧЕНИЕ - пресет с bRequiresCellShape: смена решётки
	 *  пересобирает сетку неизбежно (решётка хранится внутри неё), поэтому
	 *  такой пресет её пересоберёт и скажет об этом в статус-строку. Форма
	 *  выставляется ДО правила: пресет формы задаёт в том числе окрестность, и в
	 *  обратном порядке он затёр бы окрестность из строки правила - ровно тот
	 *  токен, ради которого привязка и заведена. Решётка при этом трогается,
	 *  только если она уже не та, что требуется. */
	UFUNCTION(BlueprintCallable, Category = "Automata|Rules")
...
private:
	...
	/** Журнал ручных правок сетки - то, что отменяет Ctrl+Z в режиме
	 *  редактирования (Tab), и одновременно СЦЕНАРИЙ, по которому та же
	 *  Ctrl+Z воспроизводит траекторию при откате поколения.
	 *
	 *  Пара к InitialStateCells выше: вдвоём они полностью описывают, как
	 *  получилось то, что сейчас на экране - изначальный узор плюс правки в
	 *  тех поколениях, где их сделали. Именно поэтому StepBackward() не хранит
	 *  ни одного снимка сетки: всё, что ему нужно, - пересев и этот список
	 *  (см. CellEditJournal и doc-comment StepBackward()).
	 *
	 *  Упорядочен по Generation по построению: записи только добавляются в
	 *  конец, а откат назад срезает хвост через CellEditJournal::TrimAfter().
	 *  Чистится там же, где GenerationSamples - в ResetGenerationCounter(),
	 *  единственной воронке всех путей "начать прогон заново": новый прогон
	 *  начинается с чистой траектории, и правки от прошлой к ней не относятся.
	 *
	 *  UPROPERTY(Transient), в отличие от соседнего InitialStateCells: это
	 *  состояние, которое НЕЛЬЗЯ пересчитать (в том и весь смысл журнала), а
	 *  реинстансинг Live Coding переносит только помеченные поля - иначе
	 *  первый же хот-патч во время PIE молча стёр бы всю историю правок. */
	UPROPERTY(Transient)
	TArray<FCellEditRecord> EditJournal;

	/** Отменённые правки, ждущие Ctrl+Shift+Z. Отдельный стек, а не индекс в
	 *  EditJournal, потому что журнал обязан оставаться СЦЕНАРИЕМ ровно того,
	 *  что сейчас в сетке: отменённая правка из траектории выбывает целиком,
	 *  иначе следующий же откат поколения воспроизвёл бы её обратно.
	 *
	 *  Сбрасывается любым новым действием (новой правкой, шагом симуляции,
	 *  откатом поколения) - стандартное поведение редакторов, и здесь оно
	 *  вдобавок обязательное: правка, снятая с поколения, которого больше нет
	 *  в траектории, накатилась бы не туда. */
	UPROPERTY(Transient)
	TArray<FCellEditRecord> EditRedoStack;

	/** Потолок веса журнала В КЛЕТКАХ (не в записях): одна запись - это одна
	 *  осознанная операция руками, но операция может быть и на миллион клеток
	 *  (выделить пол-сетки и удалить), так что считать записи бессмысленно.
	 *  При 12 байтах на клетку 5 млн - это ~100 МБ потолка.
	 *
	 *  Переполнение НЕ выбрасывает старые записи: журнал - сценарий, и запись
	 *  из его середины нельзя убрать, не сделав пересчёт неверным молча.
	 *  Вместо этого журнал помечается неполным (bEditJournalOverflowed), откат
	 *  поколения после этого отказывается работать, а отмена самих правок
	 *  продолжает - см. UndoLastAction(). */
	UPROPERTY(EditAnywhere, Category = "Automata|Editing",
		meta = (ClampMin = "0", AllowPrivateAccess = "true"))
	int64 EditJournalMaxCells = 5000000;

	/** Журнал перестал быть полным сценарием траектории - потолок веса выше
	 *  был превышен, и правку не записали. Дальше пересчёт от изначального
	 *  узора траекторию не воспроизведёт, поэтому StepBackward() отказывается
	 *  (см. её проверку), пока флаг не снимет ResetGenerationCounter(). */
	bool bEditJournalOverflowed = false;

	/** Положить правку в журнал: проверяет потолок веса, гасит стек повтора и
	 *  логирует. Единственная точка записи - через неё обязаны идти все пути
	 *  ручной правки сетки (сейчас DeleteSelectedCells(), дальше добавление
	 *  клеток), иначе журнал перестанет быть полным сценарием траектории и
	 ...
	 *  реинстансинга Live Coding нетегированный член держит мусор, а призрак,
	 *  считающий себя сдвинутым в случайную клетку, - это клетка, поставленная
	 *  не туда. */
	UPROPERTY(Transient)
	bool bPreviewNudgeActive = false;

	/** Куда сдвинут призрак и к какой грани он при этом прилип. Нормаль нужна и
	 *  дальше: по ней выбирается плоскость следующего шага, и по ней же
	 *  стирание находит клетку, на которую призрак опирается. */
	UPROPERTY(Transient)
	FIntVector PreviewNudgeCell = FIntVector::ZeroValue;

	UPROPERTY(Transient)
	FIntVector PreviewNudgeNormal = FIntVector::ZeroValue;

	/** Последнее, что посчитал UpdateCellPreview() из луча, - точка отсчёта для
	 *  первого шага стрелкой. Без неё первая стрелка не знала бы, от чего
	 *  отталкиваться: курсор к этому моменту уже никуда не указывает
	 *  осмысленно, а пересчитывать луч заново значило бы получить другую
	 *  клетку, чем та, что нарисована на экране. */
	UPROPERTY(Transient)
	FIntVector LastPreviewCell = FIntVector::ZeroValue;

	UPROPERTY(Transient)
	FIntVector LastPreviewNormal = FIntVector::ZeroValue;

	UPROPERTY(Transient)
	bool bHasLastPreview = false;

	/** Клетка, которую поставит/сотрёт следующий клик: сдвинутая стрелками, если
	 *  сдвиг активен, иначе - посчитанная из луча. Одна воронка на призрак и на
	 *  саму правку, чтобы поставленное не могло разойтись с показанным. */
	bool ResolvePaintTarget(const FVector& RayOrigin, const FVector& RayDirection, bool bErase,
							FIntVector& OutCell);
	 ...
	 *  не трогается.
	 *
	 *  Единственное, что с паттерном по дороге делается, - ПЕРЕНОС ЦЕНТРА ЕГО
	 *  ГАБАРИТА В НАЧАЛО КООРДИНАТ (AutomatonStateSerializer::
	 *  ComputeCenteringOffset(), там же почему сдвиг чётный): извлечённое
	 *  выделение хранит мировые координаты того места, где рой вырос, и без
	 *  переноса файл открывался бы за тысячи клеток от камеры. Сдвиг
	 *  применяется к КОПИИ - InitialStateCells остаётся на месте, так что R
	 *  после сохранения возвращает туда же, куда и до него; расплата -
	 *  временная копия массива координат на время записи. Обе секции файла
	 *  (снимок и InitialCells) переносятся одним и тем же сдвигом, иначе они
	 *  разъехались бы по разным системам координат.
	 *
	 *  Отказ (нет паттерна для сохранения - InitialStateCells
	...
	/** Смещения к соседям ПО ГРАНИ для текущей решётки - тем, что заслоняют
	 *  клетку. Пустой список означает "решётка неизвестна", и тогда оболочка не
	 *  скрывает ничего.
	 *
	 *  Это НЕ окрестность правила, хотя для куба они совпадают: правилу важно,
	 *  кто на кого влияет, здесь - кто кого заслоняет. */
	void BuildFaceNeighborOffsets(TArray<FIntVector>& OutOffsets) const;

	/** Закрыта ли клетка соседями со всех сторон, то есть невидима ни одним
	 *  пикселем. Сосед считается закрывающим, только если он сам НАРИСОВАН -
	 *  жив и проходит те же отсечения; иначе срез, куб и фильтр по возрасту
	 *  вскрывали бы полую внутренность. */
	bool IsCellEnclosed(const FIntVector& Cell, const TArray<FIntVector>& FaceOffsets,
	 ...
	 *  на K), продолжают звать EnsureRenderCullVolume() напрямую - область
	 *  существует независимо от того, режет она сейчас или нет. */
	...
	/** Всё, что делает ApplyCellShapePreset(), КРОМЕ пересборки сетки: поля
	 *  геометрии, меш из слота, тумблер, проверки пропорций и числа граней.
	 *  Возвращает false, если форму применить нельзя (индекс вне диапазона или
	 *  нужна скошенная решётка), не тронув при этом ни одного поля.
	 *
	 *  Разделение существует ради ApplyRulePreset(): пресет правила, привязанный
	 *  к решётке, обязан выставить сначала форму, а потом правило - иначе пресет
	 *  формы затрёт окрестность, только что разобранную из строки правила, - и при
	 *  этом заплатить за перегенерацию ОДИН раз, а не два. Сетка строится в
	 *  конце, когда все поля уже на местах. */
	bool ApplyCellShapeFields(int32 PresetIndex);

	/** Правило по текущим Birth/Survival/States и ТОЙ окрестности, которая
	...
	/** Ctrl+Z (StepBackward()), нажатый, пока bStepInProgress ещё true - третий
	 *  случай той же пары "баг и решение", что bResetToInitialStatePending и
	 *  bNewSeedPending выше, с одной особенностью: сам откат тоже занимает
	 *  bStepInProgress, и занимает надолго (пересчёт всех поколений с нуля -
	 *  см. StepBackward()), так что второе нажатие приходится на фоновый шаг
	 *  куда чаще, чем у R и N. Разряжается там же, где они: в ApplyStepResult()
	 *  и в завершении фоновой лямбды - как самого StepBackward(), так и Next().
	 *
	 *  Гасит флаги R и N и гасится ими: последнее нажатие выигрывает, потому
	 *  что все три хотят от одного Grid несовместимых вещей (изначальный узор /
	 *  новый сид / предыдущее поколение).
	 *
	 *  Взводится ОДИН раз, а не считается счётчиком: два быстрых Ctrl+Z подряд
	 *  во время долгого пересчёта откатят на одно поколение, а не на два. Это
	 *  лучше, чем очередь - каждое нажатие в ней стоило бы отдельного прогона с
	 *  нуля, и очередь из пяти на тысячном поколении встала бы на минуты. */
	bool bStepBackwardPending = false;
