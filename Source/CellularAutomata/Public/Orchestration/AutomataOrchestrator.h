...
public:
	...
	/** Снять выделение и убрать подсветку. Зовётся при ВЫХОДЕ из режима
	 *  выделения (Tab): выделение - инструмент этого режима, а не состояние
	 *  сцены, и пережившее выход подсвеченное множество потом стреляло бы в
	 *  спину - Delete, тираж и копирование молча взяли бы его вместо всей
	 *  сетки, хотя на экране уже давно другое.
	 *
	 *  Идущее перетаскивание за гизмо при этом закрывается: подсветка, уехавшая
	 *  вместе с драгом, иначе осталась бы висеть в стороне от клеток. */
	void ClearSelection();
