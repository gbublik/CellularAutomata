 ...
 *
 *  Раньше это была ОДНА структура FHudStats на полсотни полей (она ещё жива
 *  ниже, помечена deprecated, - см. её doc-comment), и держалась она одной
...
/** УСТАРЕЛА - разобрана на семь структур выше, по подсистемам (см. их общий
 *  doc-comment за тем, почему).
 *
 *  Пока жива и заполняется, чтобы существующие графы в WBP_MainHud не сломались
 *  в тот же коммит, в котором появились новые ноды. Источник правды - НЕ она:
 *  UpdateHudStats() наполняет семь новых структур, а эта копируется из них
 *  одной функцией FillLegacyHudStats(), так что разъехаться значения не могут.
 *
 *  Удаляется вместе с GetHudStats(), FillLegacyHudStats() и LastHudStats, как
 *  только последний провод в Blueprint'ах переложен на новые ноды. */
USTRUCT(BlueprintType)
struct FHudStats
{
	GENERATED_BODY()
	...
	/** Оценка объёма данных, загружаемых в GPU-буфер compute-шейдера на
	 *  последнем шаге - см. FHudPerformanceStats::EstimatedGpuComputeUploadMB. */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	double EstimatedGpuComputeUploadMB = 0.0;

	/** Сколько клеток сейчас живо во всей сетке - см.
	 *  FHudSimulationStats::AliveCellCount. */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	int32 AliveCellCount = 0;

	/** Сколько клеток сейчас в выделении - см.
	 *  FHudCameraStats::SelectedCellCount. */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	int32 SelectedCellCount = 0;

	/** Заданная скорость симуляции - см. FHudSimulationStats::SimulationSpeed. */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	float SimulationSpeed = 0.0f;

	/** Сколько поколений ЗАПРОШЕНО на один рендер - см.
	 *  FHudSimulationStats::StepsPerRender. */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	int32 StepsPerRender = 1;

	/** Сколько поколений реально продвинул последний фоновый заход - см.
	 *  FHudSimulationStats::GenerationsPerDispatch. */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	int32 GenerationsPerDispatch = 1;

	/** Текущая максимальная скорость полёта камеры - см.
	 *  FHudCameraStats::CameraSpeed. */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	float CameraSpeed = 0.0f;

	/** Ширина контура на грани клетки - см. FHudRenderStats::CellBorderWidth. */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	float CellBorderWidth = 0.0f;

	/** Идёт непрерывный прогон (пробел). */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	bool bSimulationRunning = false;

	/** Держат Shift+F - автошаг. */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	bool bFastStepActive = false;

	/** Режим взаимодействия мышью (Tab). */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	bool bSelectionModeActive = false;

	/** ВЫБРАННЫЙ метод расчёта - см. FHudPerformanceStats::ComputeMethod. */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	EComputeMethod ComputeMethod = EComputeMethod::Cpu;

	/** Видеопамять: занято сейчас и всего, МБ - см. FHudPerformanceStats. */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	double VideoMemoryUsedMB = 0.0;
	...
	/** Оперативная память: свободно и всего, МБ. */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	double SystemMemoryAvailableMB = 0.0;
	...
	/** Выбран Gpu, но последний шаг посчитан на CPU - см.
	 *  FHudPerformanceStats::bComputeFellBackToCpu. */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	bool bComputeFellBackToCpu = false;

	/** Разлитый по кадрам рендер (Z). */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	bool bChunkedRenderEnabled = false;

	/** Порядок появления клеток при разливе (X). */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	EChunkedRenderOrder ChunkedRenderOrder = EChunkedRenderOrder::Sequential;

	/** Ждать дорисовки разлива перед следующим шагом (V). */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	bool bWaitForChunkedRenderToFinish = false;

	/** Отсечение клеток по расстоянию (B) - см. FHudCutStats. */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	bool bCellCullingEnabled = false;

	/** Итог: отсечение по расстоянию СЕЙЧАС реально работает. */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	bool bCellCullingActive = false;

	/** Отсечение кубом включено (C). */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	bool bRenderCullVolumeEnabled = false;

	/** Куб виден (Ctrl+Shift+C). */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	bool bRenderCullVolumeVisible = false;

	/** Итог: клетки СЕЙЧАС реально режутся кубом. */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	bool bCullVolumeActive = false;

	/** Срез вдоль взгляда включён (J). */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	bool bViewSliceEnabled = false;

	/** Итог: срез СЕЙЧАС реально режет. */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	bool bViewSliceActive = false;

	/** Фильтр по возрасту (цифровой ряд) сейчас что-то прячет. */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	bool bAgeFilterActive = false;
	...
	/** Силуэт сейчас ЗАМЕНЯЕТ поклеточный рендер, а не дополняет его. */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	bool bGhostShapeReplacesDetailedRender = false;

	/** Имя последнего применённого профиля рендера (F1-F4). */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	FString RenderPresetName;
	...
	/** Действующее правило строкой - см. FHudSimulationStats::RuleString. */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	FString RuleString;

	/** Выбранный генератор начального состояния - см. FHudGeneratorStats. */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	FString StateGeneratorName;

	/** Сколько клеток он даст - оценка без построения. */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	int64 EstimatedGeneratorCells = 0;

	/** Клетки отбрасывают тени. */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	bool bCellsCastShadows = true;

	/** Фон (небо/туман) виден (U). */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	bool bBackgroundVisible = true;

	/** Ортогональная проекция (NumPad 5). */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	bool bOrthographicCamera = false;

	/** Лампочка на камере (Shift+U). */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	bool bHeadlightEnabled = false;

	/** Автоперебор сидов по вымиранию (Shift+N) и сколько сидов он уже перебрал. */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	bool bAutoReseedOnExtinction = false;
	...
	/** Форма кривой населения словом - см. FHudSonificationStats. */
	UPROPERTY(BlueprintReadOnly, Category = "Automata|HUD")
	FString SonificationShapeName;
};
