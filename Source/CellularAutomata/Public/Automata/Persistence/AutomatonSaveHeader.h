...
struct FAutomatonSaveHeader
{
	...
	/** Дальность соседства в клетках (см. AAutomataOrchestrator::
	 *  NeighborhoodRadius). Аддитивное поле - версия контейнера/формата не
	 *  бампается: сохранения, сделанные до появления радиуса, восстановятся
	 *  с дефолтом 1, то есть ровно с тем соседством, по которому и считались.
	 *  ApplySaveHeader() проверяет пару (соседство, радиус) на поддержанность,
	 *  а не просто клампит диапазон - JSON-шапка правится руками. */
	UPROPERTY()
