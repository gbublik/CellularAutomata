...
struct FSliceCaptureParams
{
	 ...
	 *  виден в строке состояния; и она остаётся стоять на том состоянии, с
	 *  которого серию запустили, после её окончания тоже - догоняющего рендера
	 *  нет. Экран обновится при следующем рендере (P, F, R, N).
