...
public:
	 ...
	 *  VolumeMesh, если ему назначен материал) - и ТОЛЬКО его. На отсечение не
	 *  влияет: спрятанный куб продолжает резать ровно так же, см.
	 *  AAutomataOrchestrator::GetActiveCullVolume(). Прятать коробку хочется
	 *  как раз тогда, когда она мешает разглядывать то, что она же и вырезала,
	 *  так что отсечение живёт на своей клавише (C), а видимость на своей
	 *  (Ctrl+Shift+C). Перерисовку сетки метод поэтому не запрашивает - картинка
	 *  клеток от этого флага не зависит.
	 *
	 ...
	 *
	 *  bUniformScale (Shift во время драга ручки масштаба) - тянуть все три
	 *  оси СОРАЗМЕРНО, сохраняя пропорции коробки: ось под курсором меняется
	 *  как обычно, а остальные две умножаются на то же отношение. Без него
	 *  привести коробку к нужному размеру - это три отдельных драга, после
	 *  которых пропорции всё равно уезжают. На перемещение не влияет.
	 *
	 *  bCoarseScale (Ctrl там же) - тот же драг, но куб идёт в
	 *  GizmoCoarseScaleMultiplier раз быстрее смещения курсора; в отличие от
	 *  Shift действует на ОБА вида ручек, и на масштаб, и на перемещение.
	 *  Обычный драг один к одному с миром, а коробка тут живёт в масштабе
	 *  десятков клеток по 100 юнитов - и вырастить её до размеров структуры,
	 *  и отвести на другой её край значило бы протащить ручку через полкарты
	 *  (и несколько раз перехватиться, потому что она уезжает за край экрана).
	 *  С Shift комбинируется: пропорция считается уже от ускоренного значения
	 *  ведущей оси. */
	void UpdateGizmoDrag(float AxisParam, bool bUniformScale = false, bool bCoarseScale = false);
	...
	/** Точка, через которую проходит ось активной ручки - ЗАФИКСИРОВАННАЯ в
	 *  момент захвата, а не текущая позиция актёра.
	 *
	 *  Разница здесь принципиальная, а не косметическая. Если каждый кадр
	 *  брать за начало оси текущую позицию, получается петля: кадр сдвинул
	 *  актёр на дельту -> в следующем кадре та же точка курсора отсчитывается
	 *  уже от нового начала, то есть дельта уменьшается ровно на неё же ->
	 *  актёр возвращается назад -> и так по кругу. Наблюдалось живьём как
	 *  дрожание куба между двумя точками при удержании ручки. */
	FVector GetGizmoDragOrigin() const { return DragStartActorLocation; }
...
private:
	...
	/** Во сколько раз быстрее идёт драг любой ручки с зажатым Ctrl - см.
	 *  bCoarseScale в UpdateGizmoDrag(). Не UPROPERTY
	 *  сознательно: новое reflection-поле у класса с живым инстансом на уровне
	 *  роняет редактор при LiveCoding.Compile (см. CLAUDE.md), а подбирать
	 *  число тут нечего - это ровно "x10", о котором и просили. */
	static constexpr float GizmoCoarseScaleMultiplier = 10.0f;
