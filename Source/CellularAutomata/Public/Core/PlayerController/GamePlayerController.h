...
public:
	 ...
	 *  Выключается не только Tab'ом: любая клавиша полёта (W/A/S/D/Q/E без
	 *  Ctrl) выводит из режима, см. InputKey(). */
	UFUNCTION(BlueprintCallable, Category = "Input")
	void SetSelectionModeActive(bool bActive);

	bool IsSelectionModeActive() const { return bSelectionModeActive; }

	/** Режим РИСОВАНИЯ клеток (Shift+Tab) - второй инструмент на той же "руке",
	 *  что и выделение: камера так же стоит, курсор так же виден, но мышь
	 *  означает другое. ЛКМ ставит клетку, ПКМ убирает, а под курсором всё
	 *  время висит призрак той клетки, которая встанет по нажатию (см.
	 *  AAutomataOrchestrator::UpdateCellPreview()).
	 *
	 *  Отдельный режим, а не ещё одна кнопка внутри Tab, ровно потому, что ЛКМ
	 *  не делится: в выделении она тянет рамку, здесь ставит клетку, и одно
	 *  нажатие не может означать оба. Поэтому режимы взаимоисключающи -
	 *  включение любого выключает другой.
	 *
	 *  Один клик - ОДНА клетка: рисование протяжкой было сделано и убрано по
	 *  просьбе пользователя. Точность важнее скорости набора, а цепочка клеток -
	 *  это и есть цепочка отдельных действий, каждое со своим Ctrl+Z.
	 *
	 *  ЗАЖАТЫЙ CTRL переключает мышь на буфер обмена: под курсором встаёт
	 *  призрак скопированного, и ЛКМ вставляет его целиком вместо одной клетки
	 *  (см. OnSelectDragStarted()). Одно и то же состояние решает, что показать
	 *  и что произойдёт по нажатию, - иначе призрак показывал бы одно, а клик
	 *  делал другое.
	 *
	 *  Выключается не только Shift+Tab: как и выделение, любая клавиша полёта
	 *  (W/A/S/D/Q/E без Ctrl) выводит из режима - см. InputKey(). */
	UFUNCTION(BlueprintCallable, Category = "Input")
	void SetDrawModeActive(bool bActive);

	UFUNCTION(BlueprintPure, Category = "Input")
	bool IsDrawModeActive() const { return bDrawModeActive; }
	...
	/** Перехватывает пробел (пауза) на уровне сырых оконных событий, в обход
	 *  Enhanced Input - см. подробный комментарий в реализации: при лагах
	 *  (тяжёлый AddInstances/перестройка HISM-дерева на игровом потоке) короткое
	 *  нажатие+отпускание может целиком уместиться между двумя выборками
	 *  состояния Enhanced Input за кадр и никогда не сработать.
	 *
	 *  Здесь же выход из режима выделения по клавишам полёта (W/A/S/D/Q/E без
	 *  Ctrl): в режиме ввод пешки отключён, и нажатие на них означает "хочу
	 *  лететь", а не "ничего". */
...
protected:
	 ...
	 *  вешаем свою ось на тот же ADefaultPawn::MoveUp_World - остаются E вверх и
	 *  Q вниз, т.е. движковый набор минус конфликты. Пробела там тоже нет: он
	 *  отдан паузе (см. InputKey()), а подъём и без него есть на E.
	 *
	 ...
	 *  Работает только пока непрерывная симуляция (пробел) не запущена -
	 ...
	 *  нужна. Как и пауза (см. InputKey()), R НЕ маппится через Enhanced Input -
	 ...
	 *  которого паузу понадобилось выносить из Enhanced Input) короткое
	 ...
	 *  Details panel. Как пауза и R, ловится в InputKey() в обход Enhanced Input
	 ...
	 *  Enhanced Input по той же причине, что пауза/R/N (см. его doc-comment):
	...
	/** Хоткей (P) - включить/выключить озвучивание симуляции; с зажатым Shift -
	 *  следующий набор настроек сонификации по кругу.
	 *
	 *  P - единственная честно свободная буква в проекте: она освободилась,
	 *  когда пауза переехала на пробел, и с тех пор никем не занята. Набор
	 *  повешен на Shift+P, а не на отдельную клавишу, потому что отдельных
	 *  клавиш не осталось - и потому что мьют фона это и есть голая P, вешать
	 *  на две клавиши одно действие проект уже один раз выпрямлял. */
	void OnToggleSonification();

	/** Хоткей (F10) - парадный снимок в максимальном разрешении
	 *  (AAutomataOrchestrator::TakePhotoShot()). Не путать с F6/F7: те
	 *  растеризуют сетку напрямую, без GPU, и дают плоскую картинку без света;
	 *  здесь обычный рендер движка, просто в разрешении, которое не помещается
	 *  в окно.
	 *
	 *  С зажатым Shift - сферическая панорама 360x180 из точки камеры
	 *  (TakePanoramaShot()). Модификатор проверяется внутри обработчика, как у
	 *  всех прочих Shift-пар проекта: маппинг Enhanced Input требовать
	 *  модификатор не умеет. Пара именно с F10, а не отдельная клавиша, потому
	 *  что это одно и то же действие - "сфотографировать" - с разным охватом
	 *  кадра, ровно как F6/Shift+F6 у срезов. */
	void OnTakePhotoShot();
	...
	/** Хоткей (Ctrl+Z) - отмена последнего действия, каким бы оно ни было:
	 *  AAutomataOrchestrator::UndoLastAction() сам разводит "снять ручную
	 *  правку" (обратная дельта из журнала, мгновенно) и "снять шаг симуляции"
	 *  (пересчёт от изначального узора - истории состояний в проекте нет, см.
	 *  doc-comment StepBackward()). Одна клавиша, потому что для пользователя
	 *  это одно действие, а не выбор механизма.
	 *
	...
	/** Хоткей (Ctrl+Shift+Z) - вернуть отменённую правку
	 *  (AAutomataOrchestrator::RedoLastEdit()). Только правки: "повтор шага"
	 *  это F, второй способ делать то же самое не заводим. Shift проверяется
	 *  внутри того же блока InputKey(), что и сама Ctrl+Z. */
	void OnRedoEdit();
	...
	/** Ctrl+Shift+C - показать/спрятать сам куб отсечения (ARenderCullVolume::
	...
	/** Зажат ли любой Ctrl - левый или правый. Пара к IsShiftHeld(); отдельный
	 *  метод, а не два IsInputKeyDown() по месту, потому что проверка стоит уже
	 *  в десятке обработчиков и в осях движения пешки. */
	bool IsCtrlHeld() const;

	/** Оси полёта пешки, перевешенные на контроллер, - см.
	 *  RebindPawnVerticalMovement(). Делают ровно то же, что одноимённые методы
	 *  ADefaultPawn, но молчат, пока зажат Ctrl.
	 *
	 *  Зачем: половина хоткеев проекта - это Ctrl+буква, и буквы эти те же, что
	 *  у полёта. Ctrl+D (тираж) уезжал камерой вправо, Ctrl+S (сохранение) -
	 *  назад, и выглядело это как будто нажатие "дёргает" вид. Клавиша при этом
	 *  доходит и до пешки, и до хоткея - Enhanced Input и оси движения живут
	 *  параллельно, - так что разрулить это можно только на стороне осей. */
	void PawnMoveForward(float Value);
	void PawnMoveRight(float Value);
	void PawnMoveUp(float Value);
	...
	/** Цифровой ряд - фильтр по возрасту клетки (та же цифра ещё раз снимает
	 *  фильтр). Принимает ВОЗРАСТ, а не номер клавиши: ряд сдвинут на единицу,
	 *  1 показывает возраст 0 (только что родившиеся), 9 - возраст 8, а 0
	 *  достаётся возрасту 9 ВМЕСТЕ СО ВСЕМ, ЧТО СТАРШЕ - иначе хвост рампы не
	 *  был бы виден ни под какой клавишей (см.
	 *  AAutomataOrchestrator::bAgeFilterIncludesOlder). Так ряд читается слева
	 *  направо ровно как рампа, а хвост оказывается в её конце, а не посреди.
	 *  Раскладка задаётся в HotkeyRegistry и правится из конфига.
	 *
	 ...
	 *  (см. SetSelectionModeActive()). Обратно из режима выходит ещё и любая
	 *  клавиша полёта (W/A/S/D/Q/E без Ctrl) - см. InputKey(). */
	...
	/** Ctrl+C - AAutomataOrchestrator::CopyCellsToClipboard(). Клавиша C несёт
	 *  три действия, отобранные модификаторами: голая - отсечение, Ctrl -
	 *  копирование, Ctrl+Shift - видимость куба. Копированию отдана именно
	 *  общесистемная комбинация Ctrl+C, а видимость уехала под Shift: любая
	 *  другая привязка к Ctrl+C читалась бы как чужая. Оба обработчика на этой
	 *  клавише вызываются при каждом нажатии, поэтому каждый обязан отсеять
	 *  чужие комбинации сам (Enhanced Input не умеет требовать модификатор в
	 *  маппинге). */
	void OnCopyCells();

	/** Ctrl+V - AAutomataOrchestrator::PasteClipboard() под курсором; голая V
	 *  остаётся за ожиданием чанкового рендера. Включает режим рисования, если
	 *  он выключен: вставка целится курсором, а вне режима курсора нет.
	 *
	 *  ОСНОВНОЙ способ вставки - не он, а Ctrl+ЛКМ (см. OnSelectDragStarted()):
	 *  под зажатым Ctrl призрак буфера уже стоит на месте, и просится по нему
	 *  кликнуть, а не тянуться за второй клавишей. Ctrl+V оставлен как
	 *  привычная комбинация - делает ровно то же самое. */
	void OnPasteCells();

	/** Поворот буфера на 90 перед вставкой, только в режиме рисования:
	 *  стрелки влево/вправо крутят вокруг вертикальной оси Z, вверх/вниз -
	 *  вокруг X, PageUp/PageDown - вокруг Y (пары стрелок на третью ось уже не
	 *  хватило). Стрелки делят клавиши с движением куба отсечения, но те
	 *  работают только в режиме выделения - разведены режимом, а не
	 *  модификатором (см. FHotkeyDefault::bModifierGuarded).
	 *
	 *  Крутится сам буфер, а не его отображение, так что вставится ровно то,
	 *  что показывает призрак, и ориентация переживает сколько угодно вставок
	 *  подряд. Общий код - в RotateClipboard(). */
	void OnRotateClipboardYawLeft();
	void OnRotateClipboardYawRight();
	void OnRotateClipboardPitchUp();
	void OnRotateClipboardPitchDown();
	 ...
	 *  кнопка вне рисования проекту не нужна. */
	void OnEraseCellPressed();

	/** Каждый кадр: показывает/прячет манипулятор перемещения выделения и, пока
	 *  за него тянут, продолжает драг (у ЛКМ есть только нажатие и отпускание,
	 *  само движение мыши между ними приходится опрашивать самим - как и у
	 *  куба отсечения).
	 *
	 *  Вызывается ДО раннего выхода по кубу: манипулятор выделения к кубу
	 *  отношения не имеет и обязан работать в мире, где куба нет вовсе. */
	void TickSelectionGizmo();

	/** Каждый кадр режима рисования: ведёт призрак клетки за курсором - и
	 *  больше ничего, клетки ставятся только по нажатию. Отдельный метод, а не
	 *  тело внутри Tick(), потому что вызывается ДО раннего выхода по кубу
	 *  отсечения - рисование обязано работать и в мире без куба. */
	void TickCellPainting();
	 ...
	 *
	 *  Alt+S - третье действие, QuickSaveFind(): новый файл с готовым именем,
	 *  без диалога. Проверяется раньше Ctrl и независимо от него - Alt здесь не
	 *  уточняет сохранение, а выбирает другое действие. Alt в проекте больше
	 *  нигде не используется; взят именно он, потому что обе привычные
	 *  комбинации на S уже заняты, а быстрое сохранение обязано жить на той же
	 *  клавише - это тоже "сохранить", просто не поверх. */
	void OnSaveOrSaveAs();
	 ...
	 *
	 *  Публичный, в отличие от остального ввода: спросить "какой клавишей это
	 *  делается" нужно и снаружи - легенда фильтра по возрасту на HUD
	 *  (AAutomataOrchestrator::GetAgeFilterSwatches()) подписывает квадратики
	 *  отсюда, а не зашитыми в вёрстку строками, которые после первого же
	 *  переназначения врали бы. */
public:
...
protected:
	 ...
	 *  лампочка нужна точечно, когда лезешь внутрь структуры.
	 *
	 *  UPROPERTY по той же причине, что и указатель рядом: после реинстансинга
	 *  Live Coding обычное поле осталось бы мусором, и флаг разошёлся бы с
	 *  реальной видимостью света. */
	UPROPERTY(Transient)
	...
	/** То же для режима рисования (Shift+Tab) - см. SetDrawModeActive().
	 *  Взаимоисключающ с bSelectionModeActive. */
	bool bDrawModeActive = false;
