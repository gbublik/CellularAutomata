...
protected:
	...
	/** Горит ли лампочка. По умолчанию ВКЛЮЧЕНА: без неё структура на старте
	 *  освещена только сценой, и первое, что делаешь в сессии - зажигаешь свет
	 *  Shift+U. Отдельного вызова на старте не нужно: Tick() сам зовёт
	 *  UpdateHeadlight(), пока флаг взведён, а тот создаёт компонент, как только
	 *  появится пешка (см. EnsureHeadlight()).
	 *
