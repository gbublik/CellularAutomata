...
enum class EHotkey : uint8
{
	...
	CopyCells,
	PasteCells,
	RotateClipboardYawLeft,
	RotateClipboardYawRight,
	RotateClipboardPitchUp,
	RotateClipboardPitchDown,
	RotateClipboardRollLeft,
	RotateClipboardRollRight,
...
namespace HotkeyRegistry
{
	...
	struct FHotkeyDefault
	{
		...
		FKey DefaultKey;
		/** Действие разведено с соседом по клавише чем-то, чего таблица не
		 *  видит, поэтому делить клавишу - норма, а не конфликт. Разводов два
		 *  вида, и оба живут в самих обработчиках: по МОДИФИКАТОРУ (Ctrl+Z
		 *  против голой Z) и по РЕЖИМУ (поворот буфера на стрелках работает
		 *  только в режиме рисования, движение куба отсечения - только в режиме
		 *  выделения). Проверка конфликтов такие пары пропускает; без флага она
		 *  ругалась бы на них всегда, а предупреждение, которое всегда горит,
		 *  никто не читает. */
