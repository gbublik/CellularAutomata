...
{
	// Четыре оболочки - вся модель окрестности целиком (см. ENeighborhood).
 ...
 *  (фильтр чётности, окрестность-списком, растяжение по Z). Если две применимые
...
bool FLifePatternTest::RunTest(const FString& Parameters)
{
		...
		for (const FIntPoint& Cell : Flat)
		{
			for (int32 dy = -1; dy <= 1; ++dy)
			{
				for (int32 dx = -1; dx <= 1; ++dx)
				{
					...
					// То же самое под PlanarMoore - окрестностью, ради которой
	...
	// ГЕРМЕТИЧНОСТЬ - то, ради чего заведена окрестность PlanarMoore, и то, что
	...
	//
	// Поэтому проверяются ОБЕ окрестности: под Moore-26 - фактический порог
