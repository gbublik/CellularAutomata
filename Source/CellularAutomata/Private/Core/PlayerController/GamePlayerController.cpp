...
void AGamePlayerController::SetSelectionModeActive(bool bActive)
{
	...
	// И само выделение снимается на выходе: оно инструмент этого режима, а не
	// состояние сцены. Пережившее выход подсвеченное множество потом стреляет в
	// спину - Delete, тираж (Ctrl+D) и копирование молча берут его вместо всей
	// сетки, хотя на экране давно другое, а сам выход бывает вовсе не
	// намеренным (любая клавиша полёта, см. InputKey()).
	if (!bActive)
	{
		if (AAutomataOrchestrator* Orchestrator = FindOrchestrator())
		{
