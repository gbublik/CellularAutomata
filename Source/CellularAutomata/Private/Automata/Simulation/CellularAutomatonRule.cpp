...
{
	// АНИЗОТРОПНЫЕ соседства - до маски и мимо неё: маска строится по d^2, а
	...
	// Геометрия изотропных соседств живёт в одной маске оболочек (см.
	// GetNeighborhoodShellMask()), а здесь она только разворачивается в список
	// смещений. Поэтому новое изотропное соседство - это строка там, а не ветка
	// тут.
