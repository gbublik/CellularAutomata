...
{
	// АНИЗОТРОПНЫЕ окрестности - до маски и мимо неё: маска строится по d^2, а
	// он осей не различает, так что выделить одну ось ею нельзя в принципе.
	// Единственная такая ветка на сегодня; если их станет больше, это повод
	// завести таблицу смещений, а не растить switch.
	if (InNeighborhood == ENeighborhood::PlanarMoore)
	{
		return {
			// Moore в плоскости XY - те же восемь соседей, что у двумерной
			// жизни.
			FIntVector(-1, -1, 0), FIntVector(-1, 0, 0), FIntVector(-1, 1, 0),
			FIntVector(0, -1, 0),                        FIntVector(0, 1, 0),
			FIntVector(1, -1, 0),  FIntVector(1, 0, 0),  FIntVector(1, 1, 0),
			// И ровно два соседа по Z, БЕЗ диагоналей: именно их отсутствие
			// делает перенос герметичным - клетка над двухслойной призмой
			// видит из неё только своего двойника (см. ENeighborhood::PlanarMoore).
			FIntVector(0, 0, -1),  FIntVector(0, 0, 1)
		};
	}

	// Геометрия изотропных окрестностей живёт в одной маске оболочек (см.
	// GetNeighborhoodShellMask()), а здесь она только разворачивается в список
	// смещений. Поэтому новая изотропная окрестность - это строка там, а не ветка
	// тут.
