...
namespace RulePresets
{
	...
	const TArray<FRulePreset>& GetAll()
	{
		...
		static const TArray<FRulePreset> Presets = {
			...
			// Плоская жизнь: точный перенос двумерного Конвея (B3/S23) на
			// двухслойные фигуры. Числа не подобраны, а выведены - в
			// двухслойной призме клетка видит своих соседей ДВАЖДЫ плюс
			// собственного двойника, живого ровно тогда, когда жива она сама,
			// поэтому N соседей превращаются в 2N+1 у живой клетки и в 2N у
			// мёртвой. Отсюда S = {2*2+1, 2*3+1} = {5,7}, B = {2*3} = {6}.
			//
			// Проверено в этом проекте: глайдер полетел, пульсар отпульсировал
			// и распался в те же восемь караваев, что и в 2D. Строит фигуры
			// генератор EStateGeneratorType::LifePattern (там же про порог
			// плотности, за которым перенос ломается).
			//
			// Засев случайным шаром для него смысла почти не имеет - правило
			// живёт на плоских паттернах, - но пресет обязан задать ВСЕ свои
			// поля (конвенция таблиц пресетов), поэтому радиус маленький.
			MakePreset(TEXT("Плоская жизнь (Conway 2D)"), TEXT("5,7/6/2/M"),      4,  0.3),
		};
