...
int32 FGpuComputeStrategy::StepBatch(const FCellGrid& CurrentGrid, FCellGrid& NextGrid, const FCellularAutomatonRule& Rule, int32 NumSteps) const
{
	...
	// (окрестность, радиус), дающие больше офсетов, отсекаются на входе
	// (таблица ENeighborhood замкнута на 26), так что сюда попасть можно только
	...
	//
	// Этот множитель - причина, по которой дальнобойная окрестность дороже всего
	...
	//
	// В условии ТРИ потолка, и третий - 32-битная индексация (см. проверку
	// ниже) - раньше здесь отсутствовал. Из-за этого пачка ужималась только под
	// GpuVolumeCellLimit, а выход за MAX_int32 отправлял на CPU весь заход
	// целиком - хотя пачка поменьше отлично влезала бы. Наблюдалось живьём:
	// живые занимали сторону 1277, пачка 32 давала объём 2.41 млрд и уезжала на
	// CPU, при том что пачка 6 давала 2.14 млрд и прошла бы на GPU. При 66 млн
	// клеток это разница между "GPU пачками по шесть" и "CPU целиком".
	const int64 BatchLimit = BatchVolumeCellLimit(bDecayActive);
	while (EffectiveSteps > 1
		&& (VolumeCells <= 0 || VolumeCells > MaxVolumeCells || VolumeCells > BatchLimit || VolumeCells > (int64)MAX_int32))
	{
		--EffectiveSteps;
		ComputeHaloVolume(MinAlive, MaxAlive, NeighborExtent * EffectiveSteps, MinCell, VolumeDim, VolumeCells);
	}
	...
	//
	// Здесь честный отказ вместо этого. Дойти сюда теперь можно только когда не
	// влезает ДАЖЕ ОДИНОЧНЫЙ шаг: потолок участвует в подборе размера пачки
	// выше наравне с остальными двумя, так что пачка сперва ужимается до
	// влезающей и на CPU уходит лишь то, чему на GPU места нет вовсе. Раньше
	// потолка в том условии не было, и сюда попадал весь заход целиком, стоило
	// гало раздуть объём за границу, - при том что пачка поменьше прошла бы.
	//
	// Поднять сам потолок по-настоящему - значит перевести всю индексацию на
	// int64, это отдельная работа, а не правка одной строки.
