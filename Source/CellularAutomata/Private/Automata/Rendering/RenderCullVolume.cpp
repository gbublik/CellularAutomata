...
void ARenderCullVolume::SetVolumeVisible(bool bVisible)
{
	...
	// Единственное, что должен сделать этот метод: показать/спрятать саму
	// коробку. Флаг без этого вызова только меняется в памяти - в логе всё
	// выглядит правильно, а на экране не меняется ничего.
	ApplyVolumeVisuals();

	// Перерисовывать сетку при этом НЕ нужно: видимость коробки на отсечение
	// не влияет (см. AAutomataOrchestrator::GetActiveCullVolume()), картинка
	// клеток от этого флага не зависит. Перерисовка тут когда-то стояла -
	// ровно на то время, пока спрятанный куб переставал резать, - и вместе с
	// тем инвариантом ушла: на многомиллионной сетке каждый Ctrl+Shift+C стоил бы
	// полного AddInstances() без единого изменения в картинке.
}
