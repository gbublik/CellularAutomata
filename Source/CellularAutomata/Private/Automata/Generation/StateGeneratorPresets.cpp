		...
		static const TArray<FStateGeneratorPreset> Presets = []()
		{
			...
			}
			{
				FStateGeneratorPreset P = MakePreset(
					...
					TEXT("Половина объёма в шахматном порядке. И живые, и примыкающие пустые видят по 12 соседей по Moore, так что правило приходится разводить только разницей множеств. Под соседством von Neumann вырождается: живых соседей ровно 0."),
