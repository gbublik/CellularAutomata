...
namespace
{
	...
	struct FCellEmitter
	{
		...
		FORCEINLINE bool Emit(const FIntVector& Cell)
		{
			...
			// Побитовое И вместо остатка везде ниже: координаты и их сумма бывают
			// отрицательными, а у % в C++ знак результата совпадает со знаком
			// делимого, так что "% 2 == 1" не ловит нечётные отрицательные.
			if (ParityFilter == ECellParityFilter::SameParity)
			{
				// ОЦК: все три координаты одной чётности. Условие сравнивает
				// чётности МЕЖДУ СОБОЙ, а не с константой - в отличие от
				// Even/Odd ниже, где смотрят на чётность суммы.
				const int32 ParityX = Cell.X & 1;
				if ((Cell.Y & 1) != ParityX || (Cell.Z & 1) != ParityX)
				{
					return true;
				}
			}
			else if (ParityFilter != ECellParityFilter::None)
			{
