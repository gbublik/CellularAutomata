...
## The switch, and one mesh slot per shape

`ECellShape` (`Automata/Grid/CellShape.h`) names the five solids; `CellShape` on the orchestrator is the switch. Changing it in the Details panel applies the shape *whole* — the four geometry fields, the mesh, and a rebuild of the grid — through the same `ApplyCellShapePreset()` the console command `CA.CellShape` and the HUD use. Setting a shape by hand is no longer the procedure: previously it meant writing five fields and remembering to press **Y**, since the parity filter only takes effect at *generation* time (**R** would not do — it replays `InitialStateCells` on the old lattice).

**The enum exists alongside the table rather than instead of an index.** The table is addressed by index, which suits a dropdown but nothing else: an index cannot survive a row being moved, and `FRulePreset::RequiredCellShape` has to. So the enum is the stable name, the index is a position, `FCellShapePreset::Shape` and `CellShapePresets::IndexOf()` join them, and the test `CellShape.PresetTable` asserts the two lists never drift apart.

**It is not a fifth source of truth.** The switch stores which preset was applied last — exactly what `ActiveCellShapePresetIndex` stored before it. Editing any of the four fields by hand stays legal (`Generation.ParityFilter` depends on it) and merely makes the switch stale rather than contradictory. It is `BlueprintReadOnly` for that reason: writing the field without applying the shape would change the label and nothing else. `PostEditChangeProperty()` intercepts the Details panel, Blueprint goes through `SetCellShape()`.

**Five named mesh slots, not an array.** `CubeMesh`, `HexagonalPrismMesh`, `RhombicDodecahedronMesh`, `ElongatedDodecahedronMesh`, `TruncatedOctahedronMesh` under `Automata|Cells|Meshes`. An array or a `TMap` would be shorter code, but in the panel the slots would be labelled by number, and the engine validates mesh proportions nowhere — a mesh dropped in the wrong slot would show up as gaps, i.e. as a wrongly chosen lattice. `CellMesh` remains the single field the renderer, the draw-mode preview and the clipboard ghost read; switching a shape simply *writes* the slot into it, the same idiom as `LatticeZScale` (the preset sets it and leaves, the field stays editable). An empty slot is not a refusal — the previous mesh stays, and both the log and the status line say so; the proportion check then runs against whatever mesh will actually be drawn, so an asset in the wrong slot is caught by the same assertion.

**Loading a `.casave` re-derives the shape rather than storing it.** The file holds the parity filter, the Z stretch and the offset-list neighbourhood, but no shape and no asset — deliberately, those are settings of viewing, not state of the automaton. `SyncCellShapeFromLatticeFields()` runs at the end of `ApplySaveHeader()` and identifies the shape by those three fields, restoring the switch, the mesh and `CellMeshScaleMultiplier` (which the header does not carry at all — a hole that predates the switch: a BCC pattern loaded with a multiplier of 1 draws with gaps). A combination matching no shape is left alone, since hand-assembled geometry is legal. The uniqueness of that three-field key is what `CellShape.PresetTable` checks, and it is why the hexagonal prism is skipped there: its lattice fields are the cube's.

### Two traps

**A rule string cannot change the neighbour set.** `RuleStringParser` never touches `NeighborhoodShape` — grep confirms the field is written only by `ApplyCellShapePreset()` and by save/load — while `BuildRule()` gives it priority. So the `/M` or `/CFA` token in a rule string is ignored while a lattice shape with its own offset list is active, and only Birth/Survival/States apply. `TryApplyRuleString()` now says so in the log whenever `NeighborhoodShape != Shells`, which is the only symptom this ever had; getting back to the cube means switching `CellShape` (or setting `NeighborhoodShape = Shells` by hand).

**A rule preset may demand a lattice, and exactly one does.** `FRulePreset::bRequiresCellShape` is off by default — running one rule on different lattices is the point of the whole subsystem, and a preset that silently restores the cube would take that away from all thirteen catalogue rules. It is raised only for «Плоская жизнь (Conway 2D)», whose `/PM` token would otherwise be parsed, stored and not applied (see the trap above), leaving a different rule under the same name. `ApplyRulePreset()` therefore applies the shape **before** the rule — in the other order the shape preset would overwrite the neighbourhood just parsed — parses the string ahead of both so a lattice is never rebuilt for a rule that will not apply, and pays for exactly one `GenerateState()`, which is also the one case where a rule preset destroys the current state. It says so in the status line.

**There is still no hotkey and no Details-panel button for the preset itself.** `ApplyCellShapePreset(int32)` is `CallInEditor`, but UE renders only parameterless functions as buttons. The switch replaces the need; the index-taking function stays for the console, the HUD and Blueprint.
...
**The border comes out wider than the cube's for the same `CellBorderWidth`, and that is unavoidable while the `min` is in the graph.** The `1-u` term hard-caps `u` at 0.5. A cube face is one lattice step across, so edge-to-centre is 0.5 and fits; the elongated dodecahedron's hexagonal face is a *whole* step from edge to centre. Baking the honest distance would make `1-u` win past `u > 0.96` and put a dark blob at the centre of every hexagon, so the range is compressed by half, halving the slope. Decided 2026-08-04 to keep it and scale `W` down for such shapes; the alternative — feeding `d` straight in and dropping the `min` — would additionally require regenerating the cube with the same convention, and was declined.

**The factor is 4, not the 2 this reasoning predicts, and it is measured rather than derived.** Observed 2026-08-08: at `CellBorderWidth = 0.2` every other shape matches the cube, while the elongated dodecahedron matches only at `0.05`. The extra factor of two is unexplained — worth noting that the truncated octahedron has hexagonal faces too and needs *no* correction, which is what rules out "hexagonal faces" as the cause and points at how each generator script normalised `d` instead. So the correction is a per-shape number rather than a formula: `FCellShapePreset::BorderWidthScale`, applied in `EnsureCellMaterialInstance()` on the way into the material and nowhere else. `CellBorderWidth` itself stays in units common to all shapes, so switching a lattice does not change the number under the HUD slider — the point of the whole thing being that one setting looks the same everywhere. Applying a shape re-writes the parameter, otherwise the border would keep the previous lattice's correction until the slider next moved.
...
`ApplyCellShapePreset()` already refuses it explicitly rather than applying settings that would draw hexagons on a cubic lattice, and the refusal comes *before* the mesh slot is read — no asset in `HexagonalPrismMesh` changes anything. The condition lives in `CellShapePresets::RequiresShearedLattice()`, derived from the preset's expected mesh being wider in Y than in X (a rectangular lattice cannot produce that) rather than kept as a flag: implementing the shear will retire the condition on its own, with nothing to remember to clear. It is one function because two places ask — applying a shape, and identifying one from the lattice fields — and two copies of the condition would mean one of them applying a shape the other considers unsupported. That refusal is deliberate: commit `8ef970a` reverted an earlier attempt that did the offsets first and the geometry later, which produced a picture that was plausible and wrong. The order therefore has to be the opposite — shear in `FLatticeTransform` first, with `Lattice.OrthogonalRoundTrip` extended to the sheared case, and only then the mesh.
