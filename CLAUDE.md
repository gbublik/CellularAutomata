...
- **`Automata/Simulation/`** — the rule and how a generation is computed. `ENeighborhood` is 15 values: 14 built from **four shells** within radius 2 (faces, edges, corners, far axes), plus one anisotropic — `PlanarMoore`, the XY plane plus the Z axis, which shells cannot express since `d²` does not distinguish axes. There is no separate radius; reach is derived from the offsets. Alongside it: `FCellularAutomatonRule`, the `Survival/Birth/States/Neighborhood` rule-string parser, `RulePresets`, and Generations multi-state decay (`States > 2`, costing nothing at the default 2). → **`docs/grid-and-simulation.md`**
...
- **`ConstructorHelpers::FObjectFinder` works only inside a `UObject` constructor** — anywhere else it fails a `check` and takes the Editor down. `FObjectFinderOptional` is no exception, `static` does not save it, and the crash log says nothing: it simply stops mid-line. This project creates most of its components lazily (`EnsureSelectionMeshComponent()` and friends), so the rule bites: load engine assets there with `LoadObject<UStaticMesh>(nullptr, TEXT("/Engine/BasicShapes/Cylinder.Cylinder"))` instead. Cost so far: one Editor crash on the first PIE frame.
